...
def parsefile(file):
    ...
    for row in reader:
        ...
        if row["alg"] == "ufscc"   and row["workers"] == "64":  
            addtodict(ufscc64, row["model"], row["time"])  
            addtodict(ustates, row["model"], row["ustates"])
            addtodict(tstates, row["model"], row["tstates"])
    ...
    print "\\hline"
    print " & \multicolumn{3}{c|}{\\bf number of vertices} \\\\"
    print " {{\\bf graph}} & ".format()
    print " {{\\bf unique}} & {{\\bf total explored}} & {{\\bf avg. per worker}} \\\\"
    print "\hline"
    arrr = []
    for model in sortedmodels:
        ...
        #states
        ustates_t  = mean_confidence_interval(ustates[model])
        tstates_t  = mean_confidence_interval(tstates[model])
        astates_t  = tstates_t[0] / 64
        # speedup
        ...
        pustates  = "{0:,.0f}".format(ustates_t[0])
        ptstates  = "{0:,.0f}".format(tstates_t[0])
        pastates  = "{0:,.0f}".format(astates_t)
        if other[model][0] == 100:
            pothert = " - "
            psother = " - "
        print "{{\\tt {}}} & ".format(pmodel)
        print "  {{\\tt {}}} & {{\\tt {}}} & {{\\tt {}}} \\\\".format(pustates,ptstates,pastates)
        arrr.append(starjan)
    print "\\hline"
    print "\end{tabular}"
    print "\end{document}"
